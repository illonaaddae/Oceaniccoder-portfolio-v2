...
describe('imageOptimizer utilities', () => {
  describe('isAppwriteUrl', () => {
    it('returns true for a valid Appwrite storage URL', () => {
      expect(isAppwriteUrl('https://fra.cloud.appwrite.io/v1/storage/buckets/123/files/456/view')).toBe(true);
    });
  ...
  describe('getFileIdFromUrl', () => {
    it('extracts file ID from a /view URL', () => {
      expect(getFileIdFromUrl('https://fra.cloud.appwrite.io/v1/storage/buckets/123/files/test-file-id/view')).toBe('test-file-id');
    });

    it('extracts file ID from a /preview URL', () => {
      expect(getFileIdFromUrl('https://fra.cloud.appwrite.io/v1/storage/buckets/123/files/test-file-id/preview')).toBe('test-file-id');
    });
    ...
    it('returns null if file ID cannot be found in the URL', () => {
      expect(getFileIdFromUrl('https://fra.cloud.appwrite.io/v1/storage/buckets/123/notfiles/123/view')).toBe(null);
    });
  });
  ...
  describe('getOptimizedImageUrl', () => {
    ...
    it('falls back to original URL on error during URL generation', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Failed to generate optimized image URL:", expect.any(Error));
