...
import { FaDownload, FaCertificate, FaAward, FaCalendarAlt, FaEye } from "react-icons/fa";
...
const CertificationCard = React.memo(({ cert }: CertificationCardProps) => {
  const download = getDownloadAction(cert);
  const preview = getPreviewHref(cert);
  const verify = getVerifyHref(cert);
  const actionClass =
    "flex-1 glass-btn py-2 px-3 text-sm font-medium hover:scale-105 transition-all duration-300 inline-flex items-center justify-center gap-2";
    ...
    <div className="glass-card w-full max-w-none p-6 hover:scale-105 transition-all duration-300 certification-card">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Action buttons — only rendered when there is somewhere to go. An
            anchor with no href looks like a button and does nothing. */}
          {(download || preview || verify) && (
            <div className="flex flex-col sm:flex-row gap-2">
              {download && (
                <a
                  href={download.href}
                  download={download.download}
                  className={`${actionClass} bg-gradient-to-r from-oceanic-600 to-oceanic-900 text-white`}
                >
              ...
              {preview && (
                <a
                  ...
                  className={`${actionClass} border border-oceanic-500/40 text-oceanic-500 hover:bg-oceanic-500/10`}
                >
