    ...
    return (
      <div className="mt-6 pt-6 border-t border-[var(--glass-border)]">
        ...
        <div className="relative">
          <div className={`p-4 rounded-xl border ${cardStyles} transition-opacity duration-300`}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h5 className={`text-sm font-semibold ${textPrimary}`}>{reply?.authorName}</h5>
                <p className={`text-xs ${textAccent}`}>{formatDate(reply?.$createdAt)}</p>
              </div>
            </div>
            <p className={`text-sm leading-relaxed ${textSecondary}`}>{reply?.content}</p>
          </div>
          {len > 1 && (
            <div className="flex items-center justify-center gap-3 mt-4">
              <button type="button" onClick={goPrev} className={navCls} aria-label="Previous reply">
              ...
              <button type="button" onClick={goNext} className={navCls} aria-label="Next reply">
