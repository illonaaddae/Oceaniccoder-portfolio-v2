...
const CommentForm: React.FC<CommentFormProps> = React.memo(
  ({ form, setForm, submitting, onSubmit, styles }) => {
    ...
    return (
      <form onSubmit={onSubmit} className={`p-5 md:p-6 rounded-2xl border ${cardStyles}`}>
        <h3 className={`text-lg font-semibold mb-4 ${textPrimary}`}>Leave a Comment</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            <label className={`block text-sm font-medium mb-1.5 ${textSecondary}`}>
