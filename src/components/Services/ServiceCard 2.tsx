...
import { FaCode, FaServer, FaRocket, FaCheck, FaTimes, FaStar, FaArrowRight } from "react-icons/fa";
...
const ServiceCard: React.FC<ServiceCardProps> = ({ pkg, index }) => (
  ...
  >
    ...
    <div className={`p-6 pb-0 ${pkg.popular ? "pt-10" : ""}`}>
      ...
      <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">{pkg.name}</h3>
      <p className="text-[var(--text-secondary)] text-sm mb-4 min-h-[60px]">{pkg.description}</p>
    </div>
    ...
    <ul className="space-y-3">
      {features.map((feature) => (
        <li key={feature.name} className="flex items-start gap-3">
          ...
          <span
            className={`text-sm ${
              feature.included ? "text-[var(--text-primary)]" : "text-[var(--text-accent)]"
            }`}
          >
            {feature.name}
          </span>
