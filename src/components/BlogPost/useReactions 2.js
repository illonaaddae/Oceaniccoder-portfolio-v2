...
import { getPostReactions, getVisitorReaction, addReaction } from "../../services/api";
...
export const useReactions = (postId) => {
  ...
  const handleReaction = async (reactionType) => {
    ...
    if (userReaction === reactionType) {
      ...
      if (userReaction) {
        setReactions((prev) => ({
          likes: reactionType === "like" ? prev.likes + 1 : Math.max(0, prev.likes - 1),
          dislikes: reactionType === "dislike" ? prev.dislikes + 1 : Math.max(0, prev.dislikes - 1),
        }));
      } else {
        setReactions((prev) => ({
          ...prev,
