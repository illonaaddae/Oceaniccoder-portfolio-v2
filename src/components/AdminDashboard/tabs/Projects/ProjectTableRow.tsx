...
export const ProjectTableRow: React.FC<Props> = ({ theme, project, isReadOnly, onEdit, onDelete }) => (
  <tr className={`transition-all duration-300 border-b ${theme === "dark" ? "border-white/5 hover:bg-white/5" : "border-oceanic-200/20 hover:bg-white/20"}`}>
    <td className={`px-4 py-4 transition-colors duration-300 ${theme === "dark" ? "text-white" : "text-slate-900"}`}>
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-oceanic-500 to-oceanic-500 flex items-center justify-center">
    ...
    <td className={`px-4 py-4 text-sm font-medium transition-colors duration-300 ${theme === "dark" ? "text-slate-200/90" : "text-slate-700/90"}`}>
      <div className="flex gap-2">
        {project.technologies?.slice(0, 2).map((tech) => (
          <span key={tech} className={`text-xs px-2 py-1 rounded backdrop-blur-sm border transition-colors duration-300 ${theme === "dark" ? "bg-white/10 border-white/20 text-white" : "bg-white/40 border-oceanic-200/40 text-slate-900"}`}>{tech}</span>
        ))}
      </div>
    </td>
    <td className="px-4 py-4">
