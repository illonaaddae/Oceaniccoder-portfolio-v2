...
import { getInputClass, getLabelClass, getCardClass, getHeadingClass } from "./styles";
  ...
  return (
    <div className={getCardClass(theme)}>
      ...
      <p className={`text-sm mb-4 ${theme === "dark" ? "text-slate-400" : "text-slate-500"}`}>
        These stats are shown on your About page. The "Projects Completed" count is automatically
        calculated from your projects in the database.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {statFields.map(({ key, icon: Icon, color, label, placeholder }) => (
          <div key={key}>
            <div className="flex items-center justify-between mb-2">
              <label className={`flex items-center gap-2 ${getLabelClass(theme)}`}>
