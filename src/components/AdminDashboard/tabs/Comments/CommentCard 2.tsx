import { FaCheck, FaTimes, FaReply, FaClock, FaExternalLinkAlt, FaTrash } from "react-icons/fa";
...
export const CommentCard: React.FC<CommentCardProps> = ({
  ...
  <div className={`${cardStyles} border rounded-2xl p-4 md:p-5 transition-all duration-200`}>
    {/* Header */}
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className={`font-semibold ${textPrimary} ${isReadOnly ? "blur-sm select-none" : ""}`}>
            {isReadOnly ? "••••••••" : comment.authorName}
          </h4>
          <div className="flex items-center gap-2 flex-wrap">
            {comment.authorEmail && (
              <span className={`text-xs ${textMuted} ${isReadOnly ? "blur-sm select-none" : ""}`}>
                {isReadOnly ? "••••••@••••.•••" : comment.authorEmail}
              </span>
            )}
            <span className={`text-xs ${textMuted}`}>• {formatDate(comment.$createdAt)}</span>
          </div>
        </div>
      </div>
      <span
    ...
    >
      {isReadOnly ? "Comment content is protected and only visible to admin..." : comment.content}
    </p>
