...
export function useCommentsData(isReadOnly: boolean) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const [commentsData, postsData] = await Promise.all([getAllComments(), getBlogPosts()]);
  ...
  const getPostTitle = useCallback(
    (postId: string) => blogPostMap.get(postId)?.title || "Unknown Post",
    [blogPostMap],
  );

  const getPostSlug = useCallback(
    (postId: string) => blogPostMap.get(postId)?.slug || postId,
    [blogPostMap],
  );
  ...
  const handleToggleApproval = async (commentId: string, currentStatus: boolean | undefined) => {
    if (isReadOnly) return;
    try {
      const newStatus = currentStatus === false ? true : false;
      await updateComment(commentId, { isApproved: newStatus });
      setComments((prev) =>
        prev.map((c) => (c.$id === commentId ? { ...c, isApproved: newStatus } : c)),
      );
      showToast("success", newStatus ? "Comment approved successfully!" : "Comment unapproved");
    } catch {
  ...
  const approvedComments = comments.filter((c) => c.isApproved !== false).length;
