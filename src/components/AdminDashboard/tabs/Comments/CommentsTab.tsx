import { FaComment, FaTimes, FaCheckCircle, FaExclamationCircle, FaLock } from "react-icons/fa";
...
export const CommentsTab: React.FC<CommentsTabProps> = ({ theme, isReadOnly = false }) => {
  const {
    loading,
    filter,
    setFilter,
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div>
        ...
        <p className={`text-sm sm:text-base transition-colors duration-300 ${textSecondary}`}>
          {isReadOnly
      ...
      {isReadOnly && (
        ...
        >
          <p className="text-sm font-medium flex items-center gap-2">
            ...
            Comment moderation is only available to the admin. This is a read-only view of comment
            statistics.
          </p>
        </div>
      )}
