...
const Section: React.FC<SectionProps> = ({ icon, title, gradient, children }) => (
  <div className="glass-card border border-[var(--glass-border)] rounded-2xl p-6">
    <div className="flex items-center gap-3 mb-4">
      <div className={`p-2 rounded-xl bg-gradient-to-br ${gradient}`}>{icon}</div>
...
const Text: React.FC<{ children: string }> = ({ children }) => (
  <p className="text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap">{children}</p>
);
